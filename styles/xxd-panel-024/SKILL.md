---
name: xxd-panel-024
description: "Create XXD Panel 024 artwork from supplied photos in four combinable modes: photo above/real-subject-through-geometric-emotion-window design below, photo left/design right, design alone, or a four-device wallpaper pack with independent or anchor-linked continuity. Places one photographic or finely realistic source-bound subject through one narrow horizontal, vertical, or diagonal pale low-saturation geometric block whose orientation follows subject weight and action, using generous Eastern whitespace, true subject colour, a source-derived light window, at most one faint cultural or linear support layer, and refined commercial editorial typography. Use for the exact 024 premium brand-advertising realism-through-pale-window style; never use it for black fields, line-drawn subjects, multiple windows, pasted cut-outs, cartoons, heavy dark grounds, ornate patterns, 3D, or multi-photo collage."
---

# XXD Panel 024 · 真实物象穿越几何情绪窗口

Turn each supplied photograph into one finished premium commercial editorial design. Preserve the real photograph only in paired modes; every transformed frame keeps the same subject photographic or finely realistic while it enters one narrow pale low-saturation geometric emotion window and performs one clear local crossing, extension, frame break, or passage. True subject colour, a source-derived window, Eastern whitespace, and refined type must form one light, clean, modern composition.
...
- The transformed frame must preserve identity, silhouette, pose, action, function, material, true colour, and relation through at least three source-specific cues. Choose one narrow horizontal, vertical, or diagonal pale low-saturation block from subject weight, action direction, contour flow, and page rhythm. The photographic or finely realistic subject must enter it and perform one clear local crossing. It is not a black field, line illustration, complete cut-out pasted over a stripe, multiple-window layout, cartoon, 3D render, or generic stock advert.
...
Modes and device ratios may change placement and aspect ratio but never the 024 motive. Every transformed frame must visibly express this chain:

**this exact photographed subject or inseparable relation → preserve at least three identity cues, true material and colour, and one principal action → choose one narrow horizontal, vertical, or diagonal pale low-saturation block according to subject weight, action, contour flow, and page rhythm → make the subject enter the block and perform one clear local crossing, frame break, extension, or passage → preserve photographic or finely realistic texture instead of converting it to line art → place the relation on white, pale grey, or extremely pale warm ground with generous Eastern whitespace → allow at most one barely visible cultural pattern, topographic line, ripple, light contour, or abstract linear support layer → derive the pale window from the source while keeping real subject colour and sparse neutral type → integrate one short title plus sparse supporting text along the block, subject, and whitespace relation**.

The vitality comes from a real source-bound object naturally piercing a restrained commercial geometry—not from arbitrary masking, stock luxury props, or decorative cut-outs. High-end brand advertising, luxury editorial, aesthetic magazines, premium real-estate campaigns, and seasonal visual systems are construction influences rather than templates.

Reject the result as generic when an unrelated photo could replace the source without materially changing window orientation, subject-window crossing, photographic material, true colour, focal balance, faint support layer, or copy. Also reject a pure-black field, line-drawn subject, multiple windows, complete pasted cut-out, fluorescent or broad saturation, dirty grey, global filters, ornate patterns, several focal points, cartoon, 3D volume, and template centring.

## 024 visual system

### One real subject crossing one pale window
...
- Choose one narrow horizontal, vertical, or diagonal pale low-saturation block from subject weight, action direction, contour flow, and page rhythm. It must be both visual anchor and spatial container, never a decorative stripe, huge board, several blocks, or a fixed-direction template.
...
- Keep the subject photographic or finely realistic. It enters the window and performs one clear local crossing, extension, frame break, or passage. Never paste a complete cut-out over the block. The crossing must feel naturally grown and spatially penetrating.

### Real material, true colour, and faint support

- Preserve credible photographic or finely realistic surface, light, edge, depth, and the source's actual subject colour. Do not flatten the subject into line art, silhouette, icon, or 3D-polished reconstruction.
- Use white, pale grey, or an extremely pale warm ground with generous breathing space. Allow at most one barely visible cultural pattern, topographic line, ripple, light contour, linear graphic, or abstract symbol as a subordinate layer.
- The support layer never becomes a second subject, ornate border, recognisable brand pattern, scene, or decorative wallpaper.

### Eastern whitespace and one focus

- The subject may centre, offset, grow vertically, or extend horizontally, but the window-crossing relation remains the only focus.
- Compose with light geometric order, asymmetric balance, generous Eastern whitespace, and clear primary/secondary hierarchy. Reject even filling, crowding, excessive symmetry, complex scenes, decorative borders, and template-like page furniture.

### Source-derived commercial colour system

- Extract the source's most distinctive and spirited colour relation. Soften the window into mist blue, pale cyan, ivory, powder pink, warm grey, pale gold, or another compatible light low-saturation tone.
- Preserve true subject colour; restrict type and support marks to sparse neutrals. The result stays light, clean, modern, refined, and commercially premium.
- Reject fluorescent or broad high saturation, muddy grey, heavy dark ground, global filter tints, fixed palettes unrelated to the source, and equal colour shares.

## Copy and typography belong to 024

024 text is a refined commercial editorial system belonging to the window-subject-whitespace composition—not a headline wall, UI label, signature, or detached overlay.
...
Typography has its own 024 construction:

- **Role:** reinforce the window direction, subject crossing, premium hierarchy, and whitespace rhythm.
- **Voice:** restrained, refined, lucid, vital, modern, and non-promotional.
- **Construction:** one short title plus sparse subtitle, supplied place, state word, or micro-annotation.
- **Placement:** horizontal, vertical, or following the block-subject relation; supporting type enters the field edge, crossing direction, or whitespace.
- **Material:** small light editorial sans or restrained serif equivalents with generous rhythm; no handwriting gimmick, glow, or bold sales typography.
- **Script:** use native high-end brand and editorial equivalents; never force Latin tracking, rotation, or vertical setting onto every script.
...
Use the host's built-in bitmap image-generation capability by default and follow the available `imagegen` skill when exposed. View each local source before generation. Use one generation call per distinct asset; a wallpaper pack requires four calls. “Geometric emotion window”, “photographic crossing”, “pale commercial field”, and “Eastern whitespace” describe appearance only and never authorise SVG, HTML, CSS, Canvas, diagrams, or programmatic drawing as the artwork.
...
Use only images attached to the current invocation, explicit user paths, or a previously supplied image that the current request clearly identifies as “the same image”. Never broadly scan the Desktop, workspace, `~/Desktop/xxd/xxd-panel-024/`, or historical task folders to find a substitute source. If the intended source cannot be accessed, ask for it.
...
3. Before generation, resolve copy mode and locale. If missing, ask and wait:
   ...
   1. 自动文案
      我根据原图与 024 气质创作精炼标题；请同时注明语言或地区
   2. 自定义文案
...
5. Open one fresh job boundary and reserve the next unused sibling output directory for every source and selected mode. Confirm only current user-supplied sources, inspect each separately, and read the full prompt matching the working language:
   - Chinese: [references/xxd-panel-024-prompt.zh-CN.md](references/xxd-panel-024-prompt.zh-CN.md)
   - English: [references/xxd-panel-024-prompt.en.md](references/xxd-panel-024-prompt.en.md)
6. Privately lock: the principal subject or inseparable relation; at least three identity, silhouette, pose, action, function, material, true-colour, or relation cues; one horizontal, vertical, or diagonal narrow-window orientation; subject entry and one clear local crossing; photographic/finely-realistic treatment; white/pale-grey/pale-warm ground; source-derived window colour; one optional faint support-layer role; single-focus balance; short title; sparse supporting type; and reading path. Do not invent unsupported biography, location, events, ownership, emotion, symbolism, dates, provenance, or catalogue facts.
...
10. For `independent`, generate four wallpapers directly from the source. For `linked`, generate and visually approve the anchor first; every derivative receives exactly the original source plus that same anchor. The source locks identity, content, material, and true colour; the anchor locks only pale-window direction, subject-crossing grammar, source-derived commercial palette, faint support-layer role, Eastern whitespace, single-focus balance, and typography. Never chain derivatives.
...
TYPE ROLE: PREMIUM TITLE | SUBTITLE | SUPPLIED PLACE | STATE WORD | MICRO-ANNOTATION
TYPE FORM: BLACK HANDWRITING | SLENDER SANS | RESTRAINED SERIF
TYPE PLACEMENT: WINDOW EDGE | SUBJECT CROSSING | WINDOW AXIS | WHITESPACE | SCRIPT-APPROPRIATE VERTICAL OR HORIZONTAL PATH
...
Save finished work under `~/Desktop/xxd/xxd-panel-024/` unless the user supplies another destination. Create the shared `~/Desktop/xxd/` wrapper, the skill root, and each task directory when needed.
...
- The photographic or finely realistic subject remains recognisable through at least three source-specific cues and preserves material, true colour, principal action, and inseparable relationship.
- One narrow pale low-saturation horizontal, vertical, or diagonal geometric window forms the anchor; its orientation follows subject weight, action, contour flow, and page rhythm.
- The subject enters the window and performs one clear local crossing, extension, frame break, or passage; it is never a complete cut-out pasted over a stripe.
- White, pale grey, or extremely pale warm ground and generous Eastern whitespace preserve one focus. At most one barely visible cultural or linear support layer appears and never becomes decoration or a second subject.
- Window colour is source-derived and pale; subject colour remains true; type and support marks remain sparse neutrals. No black field, fluorescent saturation, dirty grey, or global filter appears.
- Automatic copy is concise, grounded, and 024-specific; exact user wording remains verbatim. All rendered text is accurate and native to the resolved locale; text-free output contains no text or pseudo-text.
- Typography uses one short title plus sparse supporting information along the window edge, subject crossing, or whitespace. It never becomes a generic bold sales headline, UI label, signature, or detachable overlay.
...
User instructions may change subject emphasis, window orientation, subject-window entry, crossing feature, photographic/realistic treatment, pale ground, source-derived window colour, faint support layer, asymmetric balance, typographic path, or copy within 024, but do not silently relax one-photo isolation, exact paired geometry, four separate wallpaper outputs, fresh-task generation, source-hidden output rules, one-window/single-focus discipline, true subject colour, or native-language typography. Leave the premium real-subject-through-pale-window aesthetic only when the user explicitly asks to leave the 024 style.

## Provenance boundary

The user's original style brief is archived at [references/024-source.md](references/024-source.md). It records the initial 3:4 top-bottom example but does not override the operative mode and source-adaptive sizing rules. The full local 024 prompt is the production specification. Never borrow subjects, colours, copy, or compositions from samples or previous outputs.
