...
Use the host's built-in bitmap image-generation capability by default and follow the available `imagegen` skill when exposed. View each local source before generation. Use one generation call per distinct asset; a wallpaper pack requires four calls. “Origami”, “paper sculpture”, “paper-cut collage”, and “folded-paper typography” describe appearance only and never authorise SVG, HTML, CSS, Canvas, diagrams, or programmatic drawing as the artwork.
...
3. Before generation, resolve copy mode and locale. If missing, ask and wait:
   ...
   1. 自动文案
      我根据原图与 014 气质创作精炼标题；请同时注明语言或地区
   2. 自定义文案
...
5. Open one fresh job boundary and reserve the next unused sibling output directory for every source and selected mode. Confirm only current user-supplied sources, inspect each separately, and read the full prompt matching the working language:
   - Chinese: [references/xxd-panel-014-prompt.zh-CN.md](references/xxd-panel-014-prompt.zh-CN.md)
   - English: [references/xxd-panel-014-prompt.en.md](references/xxd-panel-014-prompt.en.md)
6. Privately lock: the principal subject or inseparable relation group; at least three identity, silhouette, pose, action, structure, scale, negative-shape, or relation cues; source centre of gravity; contour flow; mass ratio; action direction; fold map; main and auxiliary paper hierarchy; positive/negative space; source-derived paper palette; fibre, crease, cut-edge, thickness, and shadow material; and folded-title spatial relation. Do not invent unsupported biography, location, event, ownership, emotion, symbolism, or factual annotation.
...
10. For `independent`, generate four wallpapers directly from the source. For `linked`, generate and visually approve the anchor first; every derivative receives exactly the original source plus that same anchor. The source locks identity and content; the anchor locks only fold vocabulary, facet scale, layering and nesting logic, paper palette, fibre and edge material, natural-shadow handling, one-focus hierarchy, negative-space discipline, and folded-type construction. Never chain derivatives.
...
TYPE ROLE: READABLE FOLDED-PAPER SPATIAL STRUCTURE
TYPE PLACEMENT: ARCH AROUND FORM | EXTEND ALONG AXIS | HUG CONTOUR | SUPPORT ABOVE OR BELOW | INTERWEAVE | OVERLAP IN DEPTH | SUSPEND IN NEGATIVE SPACE
TYPE MATERIAL: FOLDED, CUT, LAYERED, OR LIGHTLY PRINTED MATTE PAPER WITH REAL EDGE AND CREASE
TYPE SCALE: ONE SHORT MAIN TITLE, SUBORDINATE MICRO-NOTES ONLY
...
The user's original style brief is archived at [references/014-source.md](references/014-source.md). It records the initial 3:4 top-bottom example but does not override the operative mode and source-adaptive sizing rules. The full local 014 prompt is the production specification. Never borrow subjects, colours, copy, or compositions from samples or previous outputs.
