...
极小图形、大笔触、少细节；暖灰白纸上炭黑主导与少量锈红，严禁多色与背景底板。
...
安装后重新启动 Agent 会话，然后调用 `$xxd-panel-170`。也可以按需追加 `--global --agent codex --yes` 做用户级安装。

常用调用示例：

```text
/xxd-panel-170 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale zh-CN
...
完整运行契约见 [SKILL.md](SKILL.md)；运行适配器见 [英文](references/xxd-panel-170-prompt.en.md) 与 [中文](references/xxd-panel-170-prompt.zh-CN.md)。
...
- `design-only`：整张画布只呈现 Panel 170 的设计转译，照片只作为不可见参考。
