...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 071 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 071 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 071 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 071 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 071 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 071 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 071 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 071 新增上下样张 4"></td>
  </tr>
</table>
...
很多照片转成插画后，要么完整描摹而显得拥挤，要么只加一层柔和滤镜，既没有保留最值得记住的关系，也没有让留白真正参与叙事。

**Panel 071** 把主体、姿态和叙事关系压缩成少量柔软、略带稚拙的形与色块，再用粉彩、粉蜡笔、水溶性色铅笔和轻薄水粉留下擦拭、透纸、叠色与停顿。元素像记忆片段一样在近白纸面上漂浮、呼应或牵引；纤细手写诗句带着笔压变化和基线漂移，自然穿过留白，而不是套进标题模板。
...
> 用 XXD Panel 071 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 071 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅近白纸面上的轻盈粉彩记忆：柔软、明亮、略带天真，却有成熟的编辑秩序 |
| **一眼可辨的特点** | 少量稚拙形体在大留白中漂浮，粉蜡笔与薄水粉保留擦拭和透纸感，纤细手写随画面游走 |
| **它如何尊重你的输入** | 保留主体身份、姿态和叙事关系；只柔化、提纯原图最有生命力且最能表达情绪的颜色 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-071`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-071-prompt.en.md) · [中文运行适配器](references/xxd-panel-071-prompt.zh-CN.md)
...
**关键词：** 柔和粉彩 · 粉蜡笔 · 水溶性色铅笔 · 近白纸面 · 漂浮记忆片段 · 诗性手写
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 071 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-071 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-071 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-071 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 071；公共设置只解析一次
/xxd-panel-071 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
71 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–071 的全部项目地址和核心风格特点；当前项目以粗体标出。
