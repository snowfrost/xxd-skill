...
<table>
  <tr>
    ...
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 016 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 016 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 016 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 016 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 016 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 016 新增上下样张 3"></td>
...
**Panel 016** 适合这类任务：需要保留原图核心内容，同时希望最终视觉达到以下方向：**把照片里真正发生的关系，压缩成一枚锚点和一场运动**。它先从用途与期望结果出发，再决定构图、尺寸、文字和具体输出形式。
...
> 用 XXD Panel 016 帮我处理这张图，先推荐最合适的构图和尺寸。
...
| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 把照片里真正发生的关系，压缩成一枚锚点和一场运动 |
| **一眼可辨的特点** | ONE SUBJECT · ONE MOTION · A LARGE FIELD OF AIR |
...
```bash
npx skills add https://github.com/nevertoday/xxd-panel-016 --skill xxd-panel-016
```
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 016 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
```text
/xxd-panel-016 photo.jpg --prefs last
/xxd-panel-016 photo.jpg --prefs edit
/xxd-panel-016 photo.jpg --prefs new
```
...
/xxd-panel-016 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN

# 左右对照：3:2 横版，无文字
/xxd-panel-016 photo.jpg --mode left-right --size 3:2 --text none

# 只要设计图：9:16 手机竖版，日语文字
/xxd-panel-016 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# 四端连贯壁纸：先建立一张定调图，再按设备分别重构
/xxd-panel-016 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# 四张独立壁纸：同时指定四台设备的准确分辨率
/xxd-panel-016 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# 跟随原图比例
/xxd-panel-016 photo.jpg --mode design-only --size source --text none

# 同一用途生成方形、竖版和横版三种完整构图
/xxd-panel-016 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# 自定义比例与准确像素可以一起多选
/xxd-panel-016 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# 模型依照原始提示词决定文字，只限定语言
/xxd-panel-016 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-016 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-016 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 016；公共设置只解析一次
/xxd-panel-016 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN

# 重复参数也会累积，并把本次任务放到指定根目录
/xxd-panel-016 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
