...
**Panel 016** is suited to work that must retain the source's essential content while pursuing this visual direction: **Compress what truly happens in a photograph into one anchor and one field of motion**. It begins with intended use and desired outcome; layout, size, text, and delivery format follow from those needs.
...
> Use XXD Panel 016 on this image and recommend the best layout and size first.
...
/xxd-panel-016 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB
...
/xxd-panel-016 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-016 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 016 to a whole directory; shared settings resolve once
/xxd-panel-016 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB
