...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 080 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 080 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 080 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 080 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 080 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 080 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 080 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 080 新增上下样张 4"></td>
  </tr>
</table>
...
普通商业插画容易把照片变成光滑模板，或者用植物、符号和背景把画面填满，却没有真正读懂照片的情绪与潜在隐喻。

**Panel 080** 以选择性记忆提取主体、结构、姿态、叙事关系和一个源自照片的微妙隐喻，再用柔性有机几何、无描边扁平色块、数字水粉／蜡笔和颗粒肌理重构。鼠尾草绿与橄榄绿构成植物色基调，少量暖色点亮情绪；大片留白承担空气、时间、距离与沉默。

### 适合这些情况

- **想把情绪变成可见的画面关系：** 通过尺度、距离、正负形与留白重构安静、温暖、孤独、期待或自由。
- **需要从照片中提炼自然隐喻：** 植物、路径、重复形体、遮挡或远处小物承担叙事，不强加新故事。
- **偏爱数字水粉与植物系配色：** 柔性有机几何、颗粒肌理、鼠尾草绿和少量暖色共同形成克制生命力。
...
> 用 XXD Panel 080 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 080 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅安静、温柔、有生命力，同时具有成熟构图与艺术出版感的情绪海报 |
| **一眼可辨的特点** | 柔性有机色块、数字水粉颗粒、植物系低饱和配色与承担叙事的大面积留白 |
| **它如何尊重你的输入** | 保留主体身份与叙事关系；隐喻和综合色温来自原图，不强加新故事 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-080`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-080-prompt.en.md) · [中文运行适配器](references/xxd-panel-080-prompt.zh-CN.md)
...
**关键词：** 柔性有机几何 · 数字水粉与蜡笔 · 颗粒肌理 · 低饱和植物色 · 自然视觉隐喻 · 情绪留白
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 080 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-080 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-080 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-080 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 080；公共设置只解析一次
/xxd-panel-080 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–080；当前项目以粗体标出。
