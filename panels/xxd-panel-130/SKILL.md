---
name: xxd-panel-130
description: "Create XXD Panel 130 raster artwork from photographs using hard-edge faceted planar reconstruction, tiny polygonal subjects, secondary halftone and photocopy texture, and vast whitespace. Use when the user invokes xxd-panel-130 or requests this style. Supports individual images, isolated directory batches, strict 50:50 comparisons, design-only outputs, and wallpapers."
---
...
Write every selected final PNG directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-130/` (or the explicit `--out` root). Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-130-prompt.zh-CN.md` and `.en.md` — delivery adapter notes matching the family runtime blocks
