...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 084 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 084 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 084 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 084 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 084 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 084 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 084 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 084 新增上下样张 4"></td>
  </tr>
</table>
...
完整描摹会让城市与场景塞满琐碎细节；粗黑轮廓和电脑滤镜又会失去空气、光感、景深与独立出版物的克制。

**Panel 084** 用黑色或深色细线提炼主体、建筑、道路与空间关系，以竖线、横线、折线、弧线和透视引导线建立几何骨架；再用 stippling／dotted texture 的疏密变化塑造天空、地面、远景、树影、灯光与主体周围的空气层次。复杂背景被主动删减，留下清楚焦点、大面积留白与少量来源明确的强调色。

### 适合这些情况

- **城市、建筑与空间关系需要被看清：** 用几何细线、透视引导线和关键轮廓提炼道路、建筑与场景结构。
- **想表现空气、光和景深而不铺满纹理：** 通过 stippling／dotted texture 的疏密渐变塑造安静层次。
...
> 用 XXD Panel 084 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 084 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅像城市随笔、建筑速写或场景结构图的现代线描海报 |
| **一眼可辨的特点** | 纤细几何线、清晰 leading lines、点描密度渐变、蓝灰暖白底与少量 spot color |
| **它如何尊重你的输入** | 保留主体、轮廓、结构、姿态和空间引导关系；颜色只取自原图最有气质的部分 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-084`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-084-prompt.en.md) · [中文运行适配器](references/xxd-panel-084-prompt.zh-CN.md)
...
**关键词：** 极简城市线描 · 几何线性骨架 · Density-based stippling · 透视引导线 · 限制性色彩 · 诗意留白
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 084 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-084 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-084 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-084 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 084；公共设置只解析一次
/xxd-panel-084 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–084；当前项目以粗体标出。
