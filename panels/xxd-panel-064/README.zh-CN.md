...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 064 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 064 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 064 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 064 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 064 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 064 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 064 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 064 新增上下样张 4"></td>
  </tr>
</table>
## 适用场景与解决的问题

旧照片、旅行记录和私人记忆需要时间感时，直接套复古滤镜往往只剩泛黄与做旧，既显得刻意，也没有说明什么值得被留下。满版剪贴则容易变成热闹的手账，失去安静和距离。

**Panel 064** 把最重要的身份、关系和情绪拆成少量旧纸片、轻线稿与文字残片。撕边、缺失、轻微错位和大面积空白共同形成可信的私人档案感，1–2 枚源图颜色只负责唤起记忆。

### 适合这些情况

- **照片本身带有记忆或档案属性：** 家庭、旅行、人物、旧物与地点需要被安静地重新整理。
- **想表达残缺，但不能丢掉辨识度：** 只删去完整场景与次要细节，核心身份和关系仍可辨认。
...
> 用 XXD Panel 064 帮我处理这张图，先推荐最合适的构图和尺寸。

上方样张可用于确认视觉方向；准备开始时，直接前往[开始使用](#开始使用)。完整模式、尺寸和参数收录在后面的折叠资料中，可按需查阅。
<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 064 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 把照片变成安静、残缺而富有时间感的旧纸拼贴档案 |
| **一眼可辨的特点** | 旧纸拼贴 · 手撕边缘 · 档案感 · 打字机小字 · 诗性留白 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-064`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-064-prompt.en.md) · [中文运行适配器](references/xxd-panel-064-prompt.zh-CN.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 064 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-064 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-064 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-064 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 064；公共设置只解析一次
/xxd-panel-064 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
