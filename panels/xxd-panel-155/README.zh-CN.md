...
安装后重新启动 Agent 会话，然后调用 `$xxd-panel-155`。也可以按需追加 `--global --agent codex --yes` 做用户级安装。

常用调用示例：

```text
/xxd-panel-155 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale zh-CN
...
- `design-only`：整张画布只呈现 Panel 155 的设计转译，照片只作为不可见参考。
