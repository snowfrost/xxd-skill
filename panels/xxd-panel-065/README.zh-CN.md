...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 065 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 065 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 065 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 065 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 065 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 065 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 065 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 065 新增上下样张 4"></td>
  </tr>
</table>
## 适用场景与解决的问题

普通线稿能交代轮廓，却常常缺少印刷节奏；随意叠加彩线又容易变成杂乱的重影，颜色与照片没有关系。真正困难的是同时保住结构、手感和有限色彩的张力。

**Panel 065** 用纤细黑线建立准确骨架，再从原图挑出两种彼此有张力的彩线。轻微错位、叠压与并行模拟手工套色和旧印刷误差，让照片变成克制、可辨、带真实手感的编辑线稿。

### 适合这些情况

- **需要用线条保住复杂结构：** 人物姿态、建筑方向、器物比例和对象关系要一眼可辨。
- **想要套色节奏，而不是多彩重影：** 黑线负责结构，两种源图彩线负责冷暖或深浅张力。
- **喜欢旧印刷的不完美：** 错位、断线、越线和修正痕迹要真实克制，不模拟故障特效。
...
> 用 XXD Panel 065 帮我处理这张图，先推荐最合适的构图和尺寸。

上方样张可用于确认视觉方向；准备开始时，直接前往[开始使用](#开始使用)。完整模式、尺寸和参数收录在后面的折叠资料中，可按需查阅。
<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 065 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 用黑色结构线与两种源图彩线制造手绘套色节奏 |
| **一眼可辨的特点** | 黑色结构线 · 双色强调线 · 手绘套色 · 旧印刷错位 · 微型排版 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-065`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-065-prompt.en.md) · [中文运行适配器](references/xxd-panel-065-prompt.zh-CN.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 065 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-065 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-065 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-065 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 065；公共设置只解析一次
/xxd-panel-065 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
