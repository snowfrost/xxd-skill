...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 091 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 091 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 091 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 091 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 091 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 091 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 091 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 091 新增上下样张 4"></td>
  </tr>
</table>
...
照片描边滤镜只复制轮廓，完成度过高的写实素描又失去停笔与留白；091 只观察到足够，并让蓝色排线承担体积、材质和叙事。

**Panel 091** 先用准确轮廓保留主体身份，再以单一蓝色铅笔或圆珠笔的方向性排线、交叉排线、重复寻找线和笔压变化，一层层织出明暗与材质。关键区域选择性细化，其余部分主动简化，让完成与未完成并存。

### 适合这些情况

- **想保留主体但主动删减背景：** 只画维持身份和场景含义的必要线索。
- **需要真实蓝笔触感：** 保留干涩颗粒、轻微压痕、笔压变化、排线方向和重复寻找线。
- **追求“观察到足够便停笔”：** 关键区域聚线，环境逐渐减少并消失在大面积纸白中。
...
> 用 XXD Panel 091 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 091 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅像独立出版旅行速写或文化杂志插画的单色蓝笔叙事线描 |
| **一眼可辨的特点** | 蓝色轮廓、方向排线、交叉排线、重复寻找线、局部细化和大片自然纸白 |
| **它如何尊重你的输入** | 保留身份、姿态与叙事关系，只删去不必描绘的元素，不使用照片描边滤镜 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-091`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-091-prompt.en.md) · [中文运行适配器](references/xxd-panel-091-prompt.zh-CN.md)
...
**关键词：** 单色蓝笔 · 方向排线 · 交叉排线 · 选择性细化 · 速写本气质 · 叙事留白
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 091 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-091 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-091 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-091 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 091；公共设置只解析一次
/xxd-panel-091 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–091；当前项目以粗体标出。
