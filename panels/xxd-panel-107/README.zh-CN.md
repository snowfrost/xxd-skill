...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 107 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 107 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 107 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 107 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 107 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 107 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 107 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 107 新增上下样张 4"></td>
  </tr>
</table>
...
- **一句文案说不完照片里的感觉：** 让来自原图的主体、动作与意象真正进入句法，承担被删掉的词。
- **不想要“完整标题＋旁边几张小画”：** 图像必须占据单词原位，读者需要沿着文字与图像共同完成句子。
- **想要轻盈、清新而不幼稚的图文设计：** 以明快柔和的色块、现代手绘编辑插画和大量留白组织视觉诗。
- **需要同一风格灵活交付：** 支持上下对照、左右对照、纯设计、多比例、四端壁纸和图片目录批量处理。

### 它替你解决什么

- 把“图片只是配图”变成“图片就是词”，解决文字与画面各说各话的问题。
- 自动从照片的情绪、动作、关系、地点或记忆生成短句，并选择最适合图像化的词。
- 用明确的阅读路径避免贴纸墙、卡片宫格和杂乱拼贴，同时保留轻松而高级的呼吸感。

### 快速开始

> 用 XXD Panel 107 帮我处理这张图，先推荐最合适的构图和尺寸。
...
| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一张现实照片与图像替词诗严格等分的海报，必须结合文字和图像词才能读完整 |
| **一眼可辨的特点** | 文字与手绘图像词沿同一阅读路径交替出现；图像词位于轻快色块中，不是旁置配图 |
...
上下模式保留原始提示词的 3:4 与上下严格 50:50；左右模式把同一等分原则映射成左右严格 50:50。纯设计与壁纸模式才替换双区容器，但不会改写图像真正替词、沿句法顺序可读的核心逻辑。每张成品只发送一个已选模式的最终契约。

## 四种可组合输出模式

模式可以单选或多选：`top-bottom`、`left-right`、`design-only`、`wallpaper-pack`；多选时，每个模式分别生成、分别拥有自己的提示词。

- `top-bottom`：原生为 3:4；若用户选择其他整画布尺寸也始终严格等高，上方现实照片 50%，下方图像替词诗 50%。
- `left-right`：画布严格等宽，左侧现实照片 50%，右侧图像替词诗 50%，两个区域都从顶边到底边，不生成第三块底栏。
- `design-only`：原图只作为不可见的身份、结构、颜色与事实依据；整张成品的每个可见元素都必须属于该 Skill 自己的设计转译语言。
- `wallpaper-pack`：每台设备分别生成完整画布的设计转译壁纸，原图不作为照片区域出现。

上下与左右成品都必须核验 50:50 双区和“无第三带”。默认一次完成整张画布；只有完整直出无法达到精确中点或用户要求原片逐像素不变时，才用确定性脚本做最后一次无损拼合，绝不把合成海报再次送入模型处理。
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 107 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。

## 文字方式

正式生图前只确认三种选择：

1. **模型根据原始提示词生成文字**：先形成完整句意，再删除适合图像化的词并以图像原位替换；被替换词不得重复印出。
2. **使用我的准确文字**：把用户原句作为底层句子，不改写、不翻译、不补标题；仅删除被选为图像词的原词，其余文字逐字保持。
3. **不要文字**：严格禁止文字与伪文字，也会主动放弃 107 的图像替词句身份，只在用户明确接受该例外时使用。
...
/xxd-panel-107 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-107 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-107 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 107；公共设置只解析一次
/xxd-panel-107 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
