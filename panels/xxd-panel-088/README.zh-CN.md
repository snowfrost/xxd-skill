...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 088 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 088 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 088 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 088 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 088 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 088 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 088 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 088 新增上下样张 4"></td>
  </tr>
</table>
...
完整描边会把主体变成普通插画，文字云又只留下随机噪声；两者都没有让语言真正承担图像结构。

**Panel 088** 让完整单词、字母、汉字、数字、标点、碎片文字与点阵成为造型材料。远看先认出照片主体，近看才发现轮廓、暗部、重量和纹理都由文字构成；疏密、字号、方向、旋转、字距和重叠共同完成一套实验字体图像系统。

### 适合这些情况

- **想让字成为图像：** 用可读词、单字符、标点和点阵共同塑造主体，而不是附加标题。
- **需要解构式排版：** 让文字聚合成暗部、拉开成空气、竖排或旋转，同时保持关键外轮廓清晰。
...
> 用 XXD Panel 088 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 088 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一张远看是主体、近看由文字与点阵组成的实验性图像海报 |
| **一眼可辨的特点** | 清晰外轮廓、文字密度梯度、旋转与断裂字符、点阵结构和主动留白 |
| **它如何尊重你的输入** | 从主体身份、动作、环境与隐喻生长文字，让字符承担轮廓、骨骼、阴影和纹理 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-088`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-088-prompt.en.md) · [中文运行适配器](references/xxd-panel-088-prompt.zh-CN.md)
...
**关键词：** 实验字体图像 · 解构排版 · 文字造型 · 点阵轮廓 · 视觉诗 · 留白节奏
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 088 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-088 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-088 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-088 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 088；公共设置只解析一次
/xxd-panel-088 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–088；当前项目以粗体标出。
