---
name: xxd-panel-153
description: "Create XXD Panel 153 raster artwork from photographs using monochrome journal doodles on light grid paper or off-white, bold continuous contours, a single source-derived ink, a tiny stamp-like focal subject and sparse handwritten notes. Use when the user invokes xxd-panel-153 or requests this style. Supports individual images, isolated directory batches, strict 50:50 comparisons, design-only outputs, and wallpapers."
---
...
Reduce even complex scenery to a few naive doodle marks and a small stamp-like anchor; do not reproduce architecture, rock hatching, or entire skyline detail merely because it is visible in the source. The canonical single-ink constraint applies to the design, not the retained photograph.
...
Write every selected final PNG directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-153/` (or the explicit `--out` root). Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-153-prompt.zh-CN.md` and `.en.md` — delivery adapter notes matching the family runtime blocks
