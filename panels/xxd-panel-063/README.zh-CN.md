...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 063 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 063 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 063 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 063 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 063 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 063 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 063 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 063 新增上下样张 4"></td>
  </tr>
</table>
## 适用场景与解决的问题

照片想获得数字感和抽象意味时，很容易滑向满屏像素、霓虹故障和装饰性 glitch：效果很强，主体与隐喻却被噪声吞掉。另一种常见结果只是把照片塞进方形蒙版，没有真正重组空间。

**Panel 063** 先寻找一个来自原图的核心轮廓或负形，把它变成主 Pixel Mask，再让关键对象、动作和环境在线性内外嵌套。轻微错位只负责节奏，柔和有限色盘与大留白负责让隐喻被看见。

### 适合这些情况

- **需要数字感，但不想做成赛博故障海报：** 像素语言服务主体和隐喻，不靠噪声制造存在感。
- **想让抽象画面仍然可读：** 一个核心 Mask 组织身份、动作、环境和负形关系。
- **喜欢克制的实验版式：** 阶梯边缘、局部错位和断裂只作节奏，柔和色盘与近白背景保持呼吸。
...
> 用 XXD Panel 063 帮我处理这张图，先推荐最合适的构图和尺寸。

上方样张可用于确认视觉方向；准备开始时，直接前往[开始使用](#开始使用)。完整模式、尺寸和参数收录在后面的折叠资料中，可按需查阅。
<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 063 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 用像素 Mask、柔和色块和负形隐喻重构照片 |
| **一眼可辨的特点** | 像素 Mask · 色块拼贴 · 负形嵌套 · 轻微 glitch · 大面积留白 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-063`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-063-prompt.en.md) · [中文运行适配器](references/xxd-panel-063-prompt.zh-CN.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 063 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-063 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-063 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-063 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 063；公共设置只解析一次
/xxd-panel-063 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
