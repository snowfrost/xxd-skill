...
等距微缩模型、压扁体块、层叠地形与低浮雕层级，限制2–4色，不做写实CG或游戏资产。
...
安装后重新启动 Agent 会话，然后调用 `$xxd-panel-166`。也可以按需追加 `--global --agent codex --yes` 做用户级安装。

常用调用示例：

```text
/xxd-panel-166 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale zh-CN
...
完整运行契约见 [SKILL.md](SKILL.md)；运行适配器见 [英文](references/xxd-panel-166-prompt.en.md) 与 [中文](references/xxd-panel-166-prompt.zh-CN.md)。
...
- `design-only`：整张画布只呈现 Panel 166 的设计转译，照片只作为不可见参考。
