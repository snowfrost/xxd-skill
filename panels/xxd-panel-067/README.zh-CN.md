...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 067 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 067 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 067 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 067 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 067 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 067 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 067 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 067 新增上下样张 4"></td>
  </tr>
</table>
## 适用场景与解决的问题

很多日常照片真正动人的不是“大场面”，而是一个姿势、一段互动或一点说不清的幽默。完整上色会让这些细节变得普通，装饰太多又会掩盖那种像随手记下来的私人感。

**Panel 067** 固定只用红、蓝两种手绘墨色，把主要动作和关系压缩成一页私人观察笔记。歪斜线条、口语短句与大片浅纸留白共同保留温柔和幽默，不把照片包装成通用插画。

### 适合这些情况

- **想记录普通生活里的有趣瞬间：** 人物、动物、物件之间的动作和关系需要成为叙事中心。
- **希望极少颜色仍有鲜明身份：** 固定红蓝双墨建立统一视觉，不从照片扩展成多色色盘。
...
> 用 XXD Panel 067 帮我处理这张图，先推荐最合适的构图和尺寸。

上方样张可用于确认视觉方向；准备开始时，直接前往[开始使用](#开始使用)。完整模式、尺寸和参数收录在后面的折叠资料中，可按需查阅。
<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 067 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 用固定红蓝双墨记录幽默、温柔的日常观察 |
| **一眼可辨的特点** | 固定红蓝 · 手绘双墨 · 私人观察 · 童真幽默 · 大面积留白 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-067`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-067-prompt.en.md) · [中文运行适配器](references/xxd-panel-067-prompt.zh-CN.md)
...
**关键词：** 固定红蓝 · 手绘双墨 · 私人观察 · 童真幽默 · 大面积留白

Panel 067 只使用红色与蓝色两种手绘墨色，在极浅纸面上以歪斜、断续、带手部误差的线条重述一个核心场景，让画面像被认真设计过的私人观察笔记。
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 067 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-067 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-067 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-067 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 067；公共设置只解析一次
/xxd-panel-067 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
