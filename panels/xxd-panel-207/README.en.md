...
# XXD Panel 207｜Airy Blue Mixed-media Collage Insert
...
Airy Blue Mixed-media Collage Insert
...
Restart the agent session after installation, then invoke `$xxd-panel-207`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-207 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
...
See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-207-prompt.en.md) or [Chinese](references/xxd-panel-207-prompt.zh-CN.md) runtime adapter.
...
- `design-only`: the full canvas contains only Panel 207's designed translation; the photograph remains a non-visible reference.
