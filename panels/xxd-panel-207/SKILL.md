---
name: xxd-panel-207
description: "Create Panel 207 raster artwork: Airy Blue Mixed-media Collage Insert. Use when the user invokes xxd-panel-207 or requests this specific visual language."
---
...
Write every selected final PNG directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-207/` (or the explicit `--out` root). Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-207-prompt.zh-CN.md` and `.en.md` — delivery adapter notes matching the family runtime blocks
