...
# XXD Panel 142｜Extended Type–Image Masks
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 142 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 142 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 142 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 142 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 142 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 142 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 142 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 142 additional top-bottom sample 4"></td>
  </tr>
</table>

Text-mode boundary: core letterforms are the artwork itself. `exact` supplies the core wording; `prompt` derives it from the source. Supporting copy may be omitted, but fully text-free output cannot preserve this style. For `none`, first clarify whether to omit only supporting copy or switch to a non-typographic style; never silently substitute styles.
...
```bash
git clone https://github.com/nevertoday/xxd-panel-142.git
npx skills add https://github.com/nevertoday/xxd-panel-142 --skill xxd-panel-142
```

Restart the agent session after installation, then invoke `$xxd-panel-142`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-142 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
/xxd-panel-142 photo.jpg --mode left-right --size 16:9 --text prompt --locale en-US
/xxd-panel-142 photo.jpg --mode design-only --size 9:16 --text prompt --locale en-US
/xxd-panel-142 ./photos --mode design-only --size auto,3:4 --text prompt --locale ja-JP
```

See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-142-prompt.en.md) or [Chinese](references/xxd-panel-142-prompt.zh-CN.md) runtime adapter.
...
Keep the source identity while redirecting composition; retain the material signature while actively leaving space. Choose exact or generated core wording (see the text-free limitation above), with single-image or recursive directory processing and the four delivery modes below.
...
- `design-only`: the full canvas contains only Panel 142's designed translation; the photograph remains a non-visible reference.
...
Modes and sizes may be combined. Supported sizes include `1:1`, `3:4`, `4:3`, `4:5`, `5:4`, `2:3`, `3:2`, `9:16`, `16:9`, `21:9`, `5:7`, `7:5`, and exact pixels. Core wording can be prompt-generated or user-exact; fully text-free output conflicts with this style. A directory is inventoried recursively and every source is isolated while sharing one set of delivery settings; final PNG files remain flat in one fresh task directory.
