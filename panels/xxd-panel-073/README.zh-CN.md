...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 073 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 073 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 073 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 073 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 073 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 073 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 073 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 073 新增上下样张 4"></td>
  </tr>
</table>
...
**Panel 073** 把主体压缩成一个被剖开的等距微缩建筑景观，以水下大陆架般的切面与空间层级呈现。精细脚手架不是点缀，而是围绕轮廓、转折和剖面真正包裹、支撑、搭建或介入主体，让画面像一项正在建构、修复、测绘或展示的建筑研究。白色纹理纸、明艳清透的源图色相和克制编辑标注，让成品兼具建筑图解的准确与艺术海报的轻盈。
...
> 用 XXD Panel 073 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 073 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅剖切式等距微缩建筑海报，理性脚手架与主体共同组成准确、克制的空间系统 |
| **一眼可辨的特点** | 主体像置于白色纹理纸上的微型立体模型，剖面层级清晰，脚手架沿结构节点展开 |
| **它如何尊重你的输入** | 保留轮廓、结构、姿态和叙事关系；保留源图色相线索，再按原文提亮、提纯、去灰 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-073`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-073-prompt.en.md) · [中文运行适配器](references/xxd-panel-073-prompt.zh-CN.md)
...
**关键词：** 等距微缩建筑 · 剖切立方体 · 水下大陆架视角 · 脚手架系统 · 手绘建筑图解 · 白色纹理纸
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 073 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-073 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-073 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-073 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 073；公共设置只解析一次
/xxd-panel-073 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
73 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–073 的全部项目地址和核心风格特点；当前项目以粗体标出。
