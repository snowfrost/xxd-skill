...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 073 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 073 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 073 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 073 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 073 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 073 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 073 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 073 additional top-bottom sample 4"></td>
  </tr>
</table>
...
No verifiable original-X samples or provenance have been supplied, so `sample-01`–`sample-04` are not displayed. The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Every work was generated independently by Panel 073 from its own original brief; no artwork from another numbered Panel is reused. The samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## What it solves

Ordinary miniature scenes often stop at cute model-making, while scaffolding becomes decorative linework with no structural role or credible spatial relationship.

**Panel 073** compresses the subject into a cut-open isometric miniature architectural landscape, using sectional depth reminiscent of an underwater continental shelf. Fine scaffolding is not decoration: it wraps, supports, constructs, repairs, surveys, or exhibits the subject around real contours, turns, and cut faces. Textured white paper, vivid source-derived hue cues, and restrained editorial notation combine architectural-diagram accuracy with the lightness of an art poster.

### Best for

- correct isometric and sectional space with legible hierarchy rather than false perspective;
- scaffolding whose members and connection nodes genuinely wrap, support, repair, or survey the subject;
- architecture-research layouts built from miniature models, fine diagram lines, textured white paper, and restrained notation;
- top-bottom, left-right, design-only, multi-ratio, wallpaper, and directory-batch delivery.

### Quick start

> Use XXD Panel 073 on this image and recommend the most suitable composition and size first.
...
|---|---|
| **What you get** | A cutaway isometric miniature-architecture poster in which rational scaffolding and subject form one accurate spatial system |
| **Recognisable signature** | A miniature dimensional model on textured white paper, with clear sectional layers and scaffolding unfolding at structural nodes |
...
```bash
git clone https://github.com/nevertoday/xxd-panel-073.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-073" ~/.codex/skills/xxd-panel-073
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-073 --skill xxd-panel-073
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-073`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-073-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-073-prompt.zh-CN.md)
...
**Signature:** isometric miniature architecture · cut cube · continental-shelf section · rational scaffolding · hand-drawn diagram · textured white paper
...
```text
/xxd-panel-073 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-073 photo.jpg --prefs last
/xxd-panel-073 photo.jpg --prefs edit
/xxd-panel-073 photo.jpg --prefs new
```
...
/xxd-panel-073 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-073 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-073 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-073 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-073 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-073 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-073 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-073 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-073 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-073 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-073 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 073 to a whole directory; shared settings resolve once
/xxd-panel-073 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-073 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
Each of the 73 Panels preserves its own original brief and aesthetic logic. The table continuously lists every project from 001 through 073 with its defining style characteristics; the current project is bold.
