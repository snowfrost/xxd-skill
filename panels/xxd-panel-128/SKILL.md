---
name: xxd-panel-128
description: "Create XXD Panel 128 raster artwork from photographs using miniature enamel medallions, cloisonne-inspired bas-relief, subtle metal rims, elegant travel typography and generous whitespace. Use when the user invokes xxd-panel-128 or requests this style. Supports individual images, isolated directory batches, strict 50:50 comparisons, design-only outputs, and wallpapers."
---
...
Write every selected final PNG directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-128/` (or the explicit `--out` root). Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-128-prompt.zh-CN.md` and `.en.md` — delivery adapter notes matching the family runtime blocks
