...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 074 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 074 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 074 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 074 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 074 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 074 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 074 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 074 新增上下样张 4"></td>
  </tr>
</table>
...
很多照片转图标只是把原图缩进圆角方框，或套上统一的 3D 材质；看起来像图标，却没有保留照片真正有力量的关系和情绪。

**Panel 074** 先找出原图最值得保留的主体、动作、关系和隐喻，再以最少的关键元素重建一枚原创的正面伪 3D／2.5D 图标。外部始终是规范、稳定的 1:1 圆角正方形；内部通过真实遮挡、体积厚度、局部浮雕和空间距离形成连续立体空间。它追求“第一眼认主体、第二眼懂关系、第三眼感受情绪”，而不是普通物体居中。
...
> 用 XXD Panel 074 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 074 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一枚可进入品牌视觉提案或图标创意评审的原创圆角伪 3D 图标 |
| **一眼可辨的特点** | 标准 1:1 圆角外框保持不变，内部主体通过遮挡、厚度、浮雕和距离形成连续正面空间 |
| **它如何尊重你的输入** | 保留主体、轮廓、姿态和核心关系；颜色完全从原图推导，再按原文提亮、净化和强化层级 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-074`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-074-prompt.en.md) · [中文运行适配器](references/xxd-panel-074-prompt.zh-CN.md)
...
**关键词：** 标准圆角方形 · 正面伪 3D／2.5D · 原图灵魂提炼 · 连续空间遮挡 · 哑光雕塑感 · 品牌图标提案
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 074 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-074 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-074 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-074 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 074；公共设置只解析一次
/xxd-panel-074 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
74 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–074 的全部项目地址和核心风格特点；当前项目以粗体标出。
