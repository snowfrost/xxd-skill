...
# XXD Panel 151｜Naive Flat Hand Drawing
...
```bash
git clone https://github.com/nevertoday/xxd-panel-151.git
npx skills add https://github.com/nevertoday/xxd-panel-151 --skill xxd-panel-151
```

Restart the agent session after installation, then invoke `$xxd-panel-151`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-151 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
/xxd-panel-151 photo.jpg --mode left-right --size 16:9 --text prompt --locale en-US
/xxd-panel-151 photo.jpg --mode design-only --size 9:16 --text none
/xxd-panel-151 ./photos --mode design-only --size auto,3:4 --text prompt --locale ja-JP
```

See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-151-prompt.en.md) or [Chinese](references/xxd-panel-151-prompt.zh-CN.md) runtime adapter.
...
- `design-only`: the full canvas contains only Panel 151's designed translation; the photograph remains a non-visible reference.
