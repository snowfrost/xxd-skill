---
name: xxd-panel-126
description: "Create XXD Panel 126 raster artwork from photographs using poetic miniature mixed-media collage, a postmark-sized subject, thin watercolour, torn handmade paper and distant bookish typography. Use when the user invokes xxd-panel-126 or requests this style. Supports individual images, isolated directory batches, strict 50:50 comparisons, design-only outputs, and wallpapers."
---
...
Write every selected final PNG directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-126/` (or the explicit `--out` root). Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-126-prompt.zh-CN.md` and `.en.md` — delivery adapter notes matching the family runtime blocks
