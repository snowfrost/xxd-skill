...
# XXD Panel 162｜Storybook Illustration on White Cutout
...
```bash
git clone https://github.com/nevertoday/xxd-panel-162.git
npx skills add https://github.com/nevertoday/xxd-panel-162 --skill xxd-panel-162
```

Restart the agent session after installation, then invoke `$xxd-panel-162`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-162 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
/xxd-panel-162 photo.jpg --mode left-right --size 16:9 --text prompt --locale en-US
/xxd-panel-162 photo.jpg --mode design-only --size 9:16 --text none
/xxd-panel-162 ./photos --mode design-only --size auto,3:4 --text prompt --locale ja-JP
```

See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-162-prompt.en.md) or [Chinese](references/xxd-panel-162-prompt.zh-CN.md) runtime adapter.
...
- `design-only`: the full canvas contains only Panel 162's designed translation; the photograph remains a non-visible reference.
