...
童趣扁平肌理插画，整块不规则白色剪纸底衬而非等宽描边，2–4暖色，小章印大留白。
...
安装后重新启动 Agent 会话，然后调用 `$xxd-panel-162`。也可以按需追加 `--global --agent codex --yes` 做用户级安装。

常用调用示例：

```text
/xxd-panel-162 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale zh-CN
...
完整运行契约见 [SKILL.md](SKILL.md)；运行适配器见 [英文](references/xxd-panel-162-prompt.en.md) 与 [中文](references/xxd-panel-162-prompt.zh-CN.md)。
...
- `design-only`：整张画布只呈现 Panel 162 的设计转译，照片只作为不可见参考。
