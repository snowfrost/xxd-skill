...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 087 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 087 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 087 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 087 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 087 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 087 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 087 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 087 新增上下样张 4"></td>
  </tr>
</table>
...
把照片直接画成普通流程图，会丢掉主体之间真实的空间关系；随机红线和满版节点，又会变成廉价的侦探板或数字网络 UI。

**Panel 087** 从照片中提取主体、轮廓、结构、姿态与叙事关系，把它们拆成少量有意义的图钉节点，并用真实线绳重新连接成可触摸的系统地图。中心关系较密，外围逐渐疏松；朱红或珊瑚红线绳在自然白纸板上形成涌现几何，同时保留针脚阴影、交叉遮挡、手写标签和真实手工误差。

### 适合这些情况

- **想理解主体之间的关系：** 把动作方向、空间位置、结构转折和环境线索变成节点与连线。
- **需要真实装置质感：** 使用图钉、棉线、纸签、针脚阴影、松紧差异和轻微遮挡，而不是电脑绘制的网络图。
...
> 用 XXD Panel 087 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 087 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一张可触摸感强、关系清晰且带研究气质的实体系统地图海报 |
| **一眼可辨的特点** | 图钉节点、朱红线绳、中心聚合、外围疏松、纸签与手写观察痕迹 |
| **它如何尊重你的输入** | 从主体、结构、姿态、方向与空间关系中提取节点和连接，不照抄原场景 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-087`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-087-prompt.en.md) · [中文运行适配器](references/xxd-panel-087-prompt.zh-CN.md)
...
**关键词：** 实体节点 · 红色线绳 · 关系映射 · 涌现几何 · 手写注释 · 研究墙式留白
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 087 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-087 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-087 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-087 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 087；公共设置只解析一次
/xxd-panel-087 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–087；当前项目以粗体标出。
