...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 087 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 087 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 087 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 087 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 087 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 087 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 087 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 087 additional top-bottom sample 4"></td>
  </tr>
</table>
...
The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Every work was generated independently by Panel 087 from its own original brief; no artwork from another numbered Panel is reused. The samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## What it solves

Drawing a photograph as a generic flowchart loses its spatial relationships; random red lines and full-page nodes become a cheap detective board or digital network UI.

**Panel 087** extracts subject, contour, structure, pose, and narrative relation, splitting them into meaningful pin-like nodes and reconnecting them with real string. A denser core, looser perimeter, red or coral links, paper tags, shadows, overlaps, and handmade variation turn the source into a tactile systems map.

### Best for

- making actions, positions, structural turns, and environmental clues visible as nodes and links;
- a physical research-wall feeling made from pins, cotton thread, paper labels, shadows, and small irregularities;
- complex but ordered geometry: one core relation, denser at the centre and more open toward the edges;
- top-bottom, left-right, design-only, multi-ratio, wallpaper, and directory-batch delivery.

### Quick start

> Use XXD Panel 087 on this image and recommend the most suitable composition and size first.
...
|---|---|
| **What you get** | A tactile systems-map poster that makes the source's relationships visible |
| **Recognisable signature** | Pin nodes, red string, central aggregation, open perimeter, paper tags, and handwritten observation marks |
| **How it respects the source** | It extracts meaningful nodes and connections from subject, structure, pose, direction, and space rather than copying the scene |
...
```bash
git clone https://github.com/nevertoday/xxd-panel-087.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-087" ~/.codex/skills/xxd-panel-087
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-087 --skill xxd-panel-087
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-087`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-087-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-087-prompt.zh-CN.md)
...
**Signature:** physical nodes · red string links · emergent geometry · handwritten annotation · research-wall tactility · meaningful whitespace
...
```text
/xxd-panel-087 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-087 photo.jpg --prefs last
/xxd-panel-087 photo.jpg --prefs edit
/xxd-panel-087 photo.jpg --prefs new
```
...
/xxd-panel-087 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-087 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-087 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-087 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-087 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-087 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-087 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-087 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-087 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-087 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-087 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 087 to a whole directory; shared settings resolve once
/xxd-panel-087 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-087 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
The current XXD Panel series runs from 001 through 112, and every Panel retains its own independent original prompt and aesthetic logic. The table below is the historical catalog as of this project’s release, listing 001 through 087; the current project is bold.
