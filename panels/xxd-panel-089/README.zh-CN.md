...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 089 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 089 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 089 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 089 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 089 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 089 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 089 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 089 新增上下样张 4"></td>
  </tr>
</table>
...
完整重画会让生活片段失去轻盈，规则宫格又会变成贴纸商品目录；089 需要主叙事与碎片之间自然呼吸。

**Panel 089** 将照片中最值得记住的主体与细节拆成若干独立小品：一个明确的主体单元承担核心叙事，其余动作、植物、器物、食物、服饰、建筑细节或偶然物件作为辅助碎片漂浮在纸面上。松散线条、水彩、彩铅和少量粉蜡笔保留纸张透底、颗粒与轻微越线，形成像认真编辑过的私人生活手账。

### 适合这些情况

- **想记录日常而非重画场景：** 保留一个主角，再提取少量动作、材质和偶然细节。
- **需要漂浮式编辑布局：** 让小插画在大小、高低、疏密和方向上自然变化，不做宫格或贴纸目录。
- **追求松弛却不幼稚：** 用细线、水彩、彩铅、粉蜡笔和成熟留白保持私人、温柔、治愈的气质。
...
> 用 XXD Panel 089 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 089 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一页有主角、有碎片、有呼吸感的私人生活手账式视觉插页 |
| **一眼可辨的特点** | 漂浮散点布局、细线水彩彩铅、暖白纸面、轻快综合色与手写注释 |
| **它如何尊重你的输入** | 保留主体身份与叙事关系，再提炼少量日常细节，不把原图重画成完整场景 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-089`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-089-prompt.en.md) · [中文运行适配器](references/xxd-panel-089-prompt.zh-CN.md)
...
**关键词：** 私人生活手账 · 漂浮式小品 · 松散手绘线 · 水彩彩铅 · 轻快综合色 · 编辑留白
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 089 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-089 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-089 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-089 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 089；公共设置只解析一次
/xxd-panel-089 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–089；当前项目以粗体标出。
