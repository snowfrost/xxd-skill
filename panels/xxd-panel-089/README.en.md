...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 089 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 089 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 089 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 089 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 089 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 089 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 089 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 089 additional top-bottom sample 4"></td>
  </tr>
</table>
...
The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Every work was generated independently by Panel 089 from its own original brief; no artwork from another numbered Panel is reused. The samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## What it solves

Redrawing a complete scene removes the lightness of remembered fragments, while a regular grid becomes a sticker catalogue.

**Panel 089** separates the most memorable subject and details into independent spot illustrations: one clear protagonist carries the story while a few actions, plants, objects, food, clothing, architecture, materials, or accidental details float around it. Loose lines, watercolour, coloured pencil, and a little pastel retain paper, grain, hesitation, and gentle colour overflow.

### Best for

- recording daily life without redrawing a complete scene;
- a floating editorial layout whose scale, height, density, and direction feel spontaneous but guided;
- loose fine lines, watercolour, coloured pencil, pastel, bright softened colour, and mature whitespace;
- top-bottom, left-right, design-only, multi-ratio, wallpaper, and directory-batch delivery.

### Quick start

> Use XXD Panel 089 on this image and recommend the most suitable composition and size first.
...
|---|---|
| **What you get** | A private-life journal page with one protagonist, memorable fragments, and breathing room |
| **Recognisable signature** | Floating spots, fine loose line, watercolour, coloured pencil, warm paper, bright soft accents, handwritten notes |
| **How it respects the source** | It preserves identity and narrative, then extracts only a few memorable daily-life details instead of redrawing the scene |
...
```bash
git clone https://github.com/nevertoday/xxd-panel-089.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-089" ~/.codex/skills/xxd-panel-089
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-089 --skill xxd-panel-089
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-089`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-089-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-089-prompt.zh-CN.md)
...
**Signature:** private lifestyle journal · floating spot illustrations · loose hand-drawn line · watercolour and coloured pencil · bright soft colour · editorial whitespace
...
```text
/xxd-panel-089 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-089 photo.jpg --prefs last
/xxd-panel-089 photo.jpg --prefs edit
/xxd-panel-089 photo.jpg --prefs new
```
...
/xxd-panel-089 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-089 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-089 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-089 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-089 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-089 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-089 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-089 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-089 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-089 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-089 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 089 to a whole directory; shared settings resolve once
/xxd-panel-089 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-089 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
The current XXD Panel series runs from 001 through 112, and every Panel retains its own independent original prompt and aesthetic logic. The table below is the historical catalog as of this project’s release, listing 001 through 089; the current project is bold.
