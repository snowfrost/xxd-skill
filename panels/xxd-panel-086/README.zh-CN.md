...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 086 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 086 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 086 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 086 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 086 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 086 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 086 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 086 新增上下样张 4"></td>
  </tr>
</table>
...
平均保留照片里的所有元素，只会让版画失去焦点；模板化复古又容易用装饰填满页面，却没有方向、尺度张力和真正有意义的空白。

**Panel 086** 只留下 3–5 个决定画面成立的视觉信息，以剪影、几何形和简洁色块重新塑造主体，再用 2–4 种源自照片的专色、dry-brush、brayer texture、dragged ink、broken ink 与 uneven coverage 形成中世纪现代主义丝网版画。构图始终追求一个明确焦点、一个主要方向和一片有意义的空白。

### 适合这些情况

- **希望照片更有视觉秩序：** 只保留 3–5 个关键视觉信息，用剪影、几何形与色块建立明确焦点。
- **想要真实版画触感：** 让干刷、滚墨、横向拖墨、飞白、断墨与不均匀覆盖成为画面语言。
- **希望颜色鲜明但克制：** 将原图最有生命力的关系压缩为 2–4 种专色，并让纸基与留白真正参与构图。
...
> 用 XXD Panel 086 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 086 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅远看先见留白与比例、近看才见粗粝油墨细节的高级版画海报 |
| **一眼可辨的特点** | 偏置剪影、2–4 种鲜明专色、几何圆形或色带、横向拖墨、飞白和断墨 |
| **它如何尊重你的输入** | 从主体、轮廓、姿态、方向与空间关系中只保留 3–5 个最关键的信息 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-086`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-086-prompt.en.md) · [中文运行适配器](references/xxd-panel-086-prompt.zh-CN.md)
...
**关键词：** 中世纪现代主义 · 限色丝网版画 · 剪影与几何秩序 · 干刷拖墨 · 断墨飞白 · 有意义的留白
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 086 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-086 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-086 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-086 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 086；公共设置只解析一次
/xxd-panel-086 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–086；当前项目以粗体标出。
