...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 106 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 106 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 106 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 106 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 106 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 106 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 106 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 106 新增上下样张 4"></td>
  </tr>
</table>
...
照片里常常有太多细节，真正让人记住的却可能只是一道轮廓、一段动作、一种距离或一块颜色。**Panel 106** 会从原图中挑出 2–4 个决定身份的视觉锚点，把它们重构成柔和、清亮而有几何秩序的像素记忆：远看是一张高级抽象构成，近看仍能认出这就是你的照片。

### 适合这些情况

- **照片很复杂，但不想丢掉身份感：** 删除次要物体与琐碎结构，只留下真正决定这张照片是谁的轮廓、姿态、方向和色彩关系。
- **想要抽象海报，却不想变成认不出的装饰：** 用规则网格、硬边色块、局部 dithering 和正负形，让主体从抽象秩序中慢慢显现。
- **喜欢清新治愈，但讨厌廉价糖果感：** 从原图重新调制奶油白、蜜桃粉、牛奶蓝、薄荷绿等柔亮有限色盘，甜而不腻。
- **需要同一风格灵活交付：** 支持上下对照、左右对照、纯设计、多比例、四端壁纸和图片目录批量处理。

### 它替你解决什么

- 不会把照片简单缩小、套滤镜或逐物像素化，而是从本次原图一次直达地重新组织视觉记忆。
- 2–4 个锚点共同组成一幅单核心画面，不会被拆成卡片、宫格或拼贴墙。
- 对照模式严格只有两个 50:50 区域：上下就是原图在上、设计在下；左右就是原图在左、设计在右，绝不多出第三条带。

### 快速开始

> 用 XXD Panel 106 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 106 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一张像“像素记忆”的高级抽象海报，删到只剩 2–4 个最重要的身份锚点 |
| **一眼可辨的特点** | 规则网格、模块化柔彩色块、硬边轮廓、局部 dithering、单一视觉核心与大面积留白 |
| **它如何尊重你的输入** | 上方现实图保持身份、结构与自然质感；下方设计虽高度概括，关键轮廓、姿态、空间方向与情绪仍准确 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-106`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-106-prompt.en.md) · [中文运行适配器](references/xxd-panel-106-prompt.zh-CN.md)
...
**关键词：** 2–4 个视觉锚点 · 几何像素 · 模块化色块 · 硬边轮廓 · 局部 dithering · 单一视觉核心 · 大面积留白 · 柔亮有限色盘
...
上下模式保留原始提示词的 3:4 与严格 50:50；左右模式把同一等分原则映射为左右严格 50:50。纯设计与壁纸模式才替换双区容器，但不改写柔彩像素记忆的转译美学。每张成品只向 GPT Image 2 发送一个已选模式的最终契约。

## 四种可组合输出模式

模式可以单选或多选：`top-bottom`、`left-right`、`design-only`、`wallpaper-pack`；多选时，每个模式分别生成、分别拥有自己的提示词。

- `top-bottom`：严格只有上下两个等高区域，各占 50%；原图在上、设计在下，文字留在下区，禁止标题条、底栏或第三分区。
- `left-right`：严格只有左右两个等宽区域，各占 50%；原图在左、设计在右，文字留在右区，绝不能转成上下结构或增加第三条带。
- `design-only`：原图只作为不可见的身份、结构、颜色与事实依据；整张成品的每个可见元素都必须属于该 Skill 自己的设计转译语言。
- `wallpaper-pack`：每台设备分别生成完整画布的设计转译壁纸，原图不作为照片区域出现。

50:50 双区是 Panel 106 自带的版式约束，不需要用户再次声明。整张成品从本次原图一次直达生成，不把中间图或失败结果送入第二轮；必要时可用中线审计核验比例，只有生成无法满足契约或用户要求原片逐像素不变时才使用确定性拼合。
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 106 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
图像模型负责整张成品的审美重构，双联也从当前原图一次直出完整画布，禁止二次风格处理。`scripts/compose_panel.py` 只保留为 50:50 中线审计、条件明确的兜底与无创尺寸校准工具，不评价审美是否成功。
...
/xxd-panel-106 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-106 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-106 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 106；公共设置只解析一次
/xxd-panel-106 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–106；当前项目以粗体标出。
