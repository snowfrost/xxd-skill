...
# 🦁 XXD Panel 106｜Pastel Pixel Memory

### Strip a photograph to its soul, then remember it again in soft-colour pixels
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 106 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 106 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 106 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 106 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 106 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 106 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 106 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 106 additional top-bottom sample 4"></td>
  </tr>
</table>
...
The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Panel 106 generates every work independently and directly from its own original source, never from another Panel or an intermediate result. Both comparison directions use exactly two 50:50 regions; the samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## Best-fit situations and problems solved

A photograph may contain too much detail, while what stays in memory is only a contour, a movement, a distance, or a patch of colour. **Panel 106** selects 2–4 identity-defining visual anchors and rebuilds them as a soft, luminous pixel memory with clear geometric order: from afar it reads as a refined abstract composition; up close it is still unmistakably your photograph.

### Best for

- **A complex photograph that must keep its identity:** remove secondary objects and fussy structure while retaining the contour, pose, direction, and colour relationships that make the source itself.
- **An abstract poster that must not become anonymous decoration:** use regular grids, hard-edged blocks, local dithering, and positive–negative forms so the subject gradually emerges from the abstraction.
- **A fresh, soothing palette without cheap candy sweetness:** remix cream white, peach pink, milk blue, mint green, and related source colours into a soft, bright limited palette.
...
- The photograph is not merely reduced, filtered, or pixelated object by object; every result is rebuilt directly from the current original source in one pass.
- The 2–4 anchors share one visual core instead of becoming cards, a grid, or a collage wall.
- Comparison modes contain exactly two 50:50 regions: top-bottom means reality above and design below; left-right means reality left and design right, with no third band.

### Quick start

> Use XXD Panel 106 on this image and recommend the most suitable composition and size first.
...
|---|---|
| **What you get** | A refined abstract “pixel memory” poster reduced to the 2–4 anchors that matter most |
| **Recognisable signature** | Regular grids, modular pastel blocks, hard edges, local dithering, one visual core, and generous negative space |
| **How it respects the source** | The reality view keeps identity, structure, and natural texture; the transformed view simplifies radically while preserving the decisive contour, pose, spatial direction, and feeling |
...
```bash
git clone https://github.com/nevertoday/xxd-panel-106.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-106" ~/.codex/skills/xxd-panel-106
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-106 --skill xxd-panel-106
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-106`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-106-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-106-prompt.zh-CN.md)
...
**Signature:** 2–4 visual anchors · geometric pixels · modular colour blocks · hard-edged contours · local dithering · one visual core · generous negative space · soft luminous limited palette
...
Top-bottom preserves the source brief's 3:4 canvas and exact 50:50 split; left-right maps the same equal-pair principle to an exact 50:50 vertical split. Only design-only and wallpaper modes replace the paired container, without rewriting the soft pixel-memory aesthetic. Each asset sends GPT Image 2 exactly one selected mode contract.
...
- `top-bottom`: exactly two equal-height 50:50 regions, reality above and design below. Type stays inside the lower region; header bands, footers, and third regions are forbidden.
- `left-right`: exactly two equal-width 50:50 regions, reality left and design right. Type stays inside the right region; it must never rotate into top-bottom or add a third band.
...
The 50:50 two-region split is intrinsic to Panel 106 and does not need to be requested again. Every canvas is generated directly from the current original source in one pass, never from an intermediate or failed result. A midpoint audit may verify the split; deterministic assembly is reserved for a failed geometry contract or pixel-identical source preservation.
...
The image model owns the aesthetics of the entire finished composition; paired layouts are generated as one complete canvas directly from the current original source, with no second restyling pass. `scripts/compose_panel.py` remains only for 50:50 midpoint auditing, condition-based recovery, and lossless pixel calibration; it does not judge aesthetic success.
...
```text
/xxd-panel-106 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-106 photo.jpg --prefs last
/xxd-panel-106 photo.jpg --prefs edit
/xxd-panel-106 photo.jpg --prefs new
```
...
/xxd-panel-106 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-106 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-106 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-106 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-106 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-106 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-106 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-106 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-106 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-106 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-106 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 106 to a whole directory; shared settings resolve once
/xxd-panel-106 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-106 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
The current XXD Panel series runs from 001 through 112, and every Panel retains its own independent original prompt and aesthetic logic. The table below is the historical catalog as of this project’s release, listing 001 through 106; the current project is bold.
