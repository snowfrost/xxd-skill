---
name: xxd-panel-171
description: "Create Panel 171 raster artwork: Vertical Skeleton and Tiny Vignettes. Use when the user invokes xxd-panel-171 or requests this specific visual language."
---
...
Let 75–85% clean warm near-white space lead. Extract a few thin low-contrast vertical structural elements from the source, with very sparse tiny island-like narrative motifs between them. Use light grey-cyan/green structure, soft ochre/khaki/earth gold and tiny source-grounded accent spots. Avoid dense scenes, thick verticals, large saturated fields or high contrast.
...
Write every selected final PNG directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-171/` (or the explicit `--out` root). Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-171-prompt.zh-CN.md` and `.en.md` — delivery adapter notes matching the family runtime blocks
