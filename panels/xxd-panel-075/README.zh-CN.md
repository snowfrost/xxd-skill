...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 075 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 075 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 075 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 075 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 075 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 075 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 075 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 075 新增上下样张 4"></td>
  </tr>
</table>
...
很多“手绘复古”处理不是把照片描得过满，就是统一套一层旧纸滤镜：主体还在，停顿、距离与私人记忆却不见了。

**Panel 075** 把主体最关键的比例、方向、结构与姿态保留下来，用松散、略带修正痕迹的深色蜡笔线重新落笔；一块从原图综合色调中生长出的低饱和不规则色域只负责承托，米白手工纸和大面积空白则承担情绪、距离与时间。少量打字机小字像私人注记，不抢走画面的呼吸。
...
> 用 XXD Panel 075 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 075 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅像私人独立出版物内页的克制纸面速写海报 |
| **一眼可辨的特点** | 深色蜡笔线落在米白手工纸与单块柔和色域上，并带轻微 Risograph 颗粒 |
| **它如何尊重你的输入** | 省略细节但保留关键比例、方向、结构、姿态与关系；色块严格按原始提示词呼应照片综合色调 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-075`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-075-prompt.en.md) · [中文运行适配器](references/xxd-panel-075-prompt.zh-CN.md)
...
**关键词：** 深色蜡笔速写 · 米白手工纸 · 柔和不规则色域 · Risograph 颗粒 · 大面积留白 · 打字机私人注记
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 075 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-075 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-075 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-075 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 075；公共设置只解析一次
/xxd-panel-075 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
75 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–075 的全部项目地址和核心风格特点；当前项目以粗体标出。
