...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 083 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 083 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 083 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 083 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 083 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 083 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 083 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 083 新增上下样张 4"></td>
  </tr>
</table>
...
把照片完整描一遍，通常只得到乏味线稿；把它画得太漂亮、太正确，又会失去原对象最有趣的性格、动作、反差和尴尬感。

**Panel 083** 先重新判断照片，只留下一个最值得记住的主体和至多一两个必要环境暗示，再把某个比例、动作、表情或结构控制性地画歪、画笨、画得有点荒诞。发抖轮廓、故意失准比例、粗糙蜡笔与留白共同形成 Ugly-cute naïve doodle；身份必须准确，幽默必须来自原图，而不是随意加梗。
...
> 用 XXD Panel 083 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 083 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅像独立出版丑萌涂鸦小志的幽默角色海报 |
| **一眼可辨的特点** | 发抖不均匀轮廓、故意失准比例、少量粗糙上色、一个主体与大片浅色留白 |
| **它如何尊重你的输入** | 主体身份与幽默判断来自原图；只夸张最有性格的动作、反差、表情或结构 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-083`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-083-prompt.en.md) · [中文运行适配器](references/xxd-panel-083-prompt.zh-CN.md)
...
**关键词：** Ugly-cute naïve doodle · Wonky 手绘 · 故意画丑 · 单一幽默主角 · 蜡笔彩铅颗粒 · 少怪笨准
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 083 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-083 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-083 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-083 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 083；公共设置只解析一次
/xxd-panel-083 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–083；当前项目以粗体标出。
