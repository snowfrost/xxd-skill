---
name: xxd-panel-110
description: "Turn each source photograph into a strict 50:50 top-bottom or left-right poster whose transformed half becomes a realistic acrylic-collectible scene map of 4–7 source-grounded life fragments with gentle labels and dynamic whitespace. Use when the user invokes xxd-panel-110 or requests this exact independent Panel style."
---
...
Write every selected final PNG directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-110/` (or the explicit `--out` root). Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-110-prompt.zh-CN.md` and `.en.md` — delivery adapter notes matching the family runtime blocks
