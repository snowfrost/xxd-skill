...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 066 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 066 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 066 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 066 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 066 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 066 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 066 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 066 新增上下样张 4"></td>
  </tr>
</table>
## 适用场景与解决的问题

家庭、宠物、旅行和日常互动很适合画成轻松插画，但常见卡通模板会把人物关系换成统一表情，或者用过多可爱元素填满画面。结果有“童趣”，却没有原照片里的动作和故事。

**Panel 066** 保留最值得记住的主体关系，用略显笨拙的黑线与 3–6 种柔和平涂色重述场景。它允许抖动、停顿和修正，又用成熟的重心、节奏和留白控制画面，让童真不等于幼稚。

### 适合这些情况

- **想记录家庭、宠物、旅行或日常互动：** 主体动作与关系比完整背景更重要。
- **喜欢童真感，但不想要模板化可爱：** 概括比例和松弛黑线保留真实观察，不统一成卡通表情。
- **希望画面轻松又有设计判断：** 3–6 种源图柔色、偏置主体和大留白共同建立成熟节奏。
...
> 用 XXD Panel 066 帮我处理这张图，先推荐最合适的构图和尺寸。

上方样张可用于确认视觉方向；准备开始时，直接前往[开始使用](#开始使用)。完整模式、尺寸和参数收录在后面的折叠资料中，可按需查阅。
<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 066 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 用松弛黑线和柔和平涂讲述童真而克制的照片故事 |
| **一眼可辨的特点** | 童真叙事 · 笨拙黑线 · 柔和平涂 · 治愈色彩 · 手写观察 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-066`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-066-prompt.en.md) · [中文运行适配器](references/xxd-panel-066-prompt.zh-CN.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 066 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-066 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-066 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-066 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 066；公共设置只解析一次
/xxd-panel-066 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
