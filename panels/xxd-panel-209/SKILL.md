---
name: xxd-panel-209
description: "Create Panel 209 raster artwork: Minimal Paper-based Image Archive. Use when the user invokes xxd-panel-209 or requests this specific visual language."
---
...
Write every selected final PNG directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-209/` (or the explicit `--out` root). Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-209-prompt.zh-CN.md` and `.en.md` — delivery adapter notes matching the family runtime blocks
