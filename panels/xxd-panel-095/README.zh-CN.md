...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 095 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 095 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 095 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 095 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 095 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 095 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 095 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 095 新增上下样张 4"></td>
  </tr>
</table>
...
照片中的复杂对象常常需要被解释，而不是被装饰。095 将主体压缩为清楚的图解语法，让结构、动作和象征关系可读。

**Panel 095** 以 Mid-century modern diagrammatic editorial illustration 为核心：用 visual shorthand、pictographic simplification、细线、弧线、图标、局部剖面与引导线，保留决定身份的轮廓、节点、方向、动作和关系。黑色承担绝大多数信息，朱红／珊瑚红只作为精确 spot color；暖白纸、油墨颗粒、套印偏差与 overprint 让它像早期信息设计和独立出版物中的图版。
...
> 用 XXD Panel 095 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 095 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅理性、清晰、聪明且有人情味的黑红双色编辑图解 |
| **一眼可辨的特点** | 黑色技术线、红色精确标记、图标化抽象、引导线、模块错位与纸张油墨质感 |
| **它如何尊重你的输入** | 保留主体身份、结构、动作和象征关系，只删除妨碍解释的细节 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-095`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-095-prompt.en.md) · [中文运行适配器](references/xxd-panel-095-prompt.zh-CN.md)
...
**关键词：** 技术细线 · 图解简化 · 黑红专色 · 模块化信息 · 大面积留白 · 旧式印刷肌理
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 095 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-095 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-095 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-095 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 095；公共设置只解析一次
/xxd-panel-095 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–095；当前项目以粗体标出。
