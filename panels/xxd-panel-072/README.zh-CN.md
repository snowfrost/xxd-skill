...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 072 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 072 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 072 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 072 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 072 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 072 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 072 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 072 新增上下样张 4"></td>
  </tr>
</table>
...
很多“毛玻璃风”只是在整张图上加同一种模糊，主体失去识别度，画面也容易变成廉价塑料滤镜。

**Panel 072** 把主体最有辨识度的轮廓、结构、姿态和关系压缩成简洁几何、平面色块与纤细线条，再建立一套区域差异化的半透明磨砂视窗：有的局部清晰，有的柔焦，有的透叠。主体始终一眼可认，却像悬浮在安静、洁净的近白空间中；少量现代编辑文字让它更接近建筑海报、艺术展览或高端品牌视觉。

### 适合这些情况

- **想要毛玻璃但不要滤镜感：** 模糊按区域和层次设计，不是整块平均虚化。
- **需要抽象后仍然一眼可认：** 核心轮廓、结构、姿态和叙事关系不会被装饰吞没。
- **偏爱现代视觉系统：** 居中平衡、正负形、留白、构图线和克制文字共同建立专业秩序。
...
> 用 XXD Panel 072 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 072 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅高级、现代、克制的雾面玻璃抽象海报，兼具清晰识别与朦胧空气感 |
| **一眼可辨的特点** | 几何化主体悬浮在大留白中，不同区域呈现不同强度的柔焦、透叠与清晰边界 |
| **它如何尊重你的输入** | 保留核心轮廓、结构、姿态和关系；沿用原图色相身份，再按原文提亮、净化和柔化 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-072`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-072-prompt.en.md) · [中文运行适配器](references/xxd-panel-072-prompt.zh-CN.md)
...
**关键词：** 半透明磨砂视窗 · 区域差异化柔焦 · 极简几何 · 清晰识别轮廓 · 明艳治愈色 · 现代编辑排版
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 072 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-072 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-072 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-072 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 072；公共设置只解析一次
/xxd-panel-072 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
72 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–072 的全部项目地址和核心风格特点；当前项目以粗体标出。
