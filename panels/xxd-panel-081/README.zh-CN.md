...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 081 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 081 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 081 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 081 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 081 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 081 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 081 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 081 新增上下样张 4"></td>
  </tr>
</table>
...
普通描边会机械复制所有边缘，黑色外轮廓加填色又容易变成卡通贴纸；元素平均排列时，照片真正值得记住的身份与情绪反而消失。

**Panel 081** 用接近等粗的 monoline 单线体系，把主体、轮廓、姿态和关系压缩成少量记忆符号。开放轮廓、断续边缘、轻微抖动与错位消除机械矢量感；2–4 种从原图提取的彩色线条直接编码信息，以线密度、明度和专色层级建立前后，并用颗粒、套印偏差和不均匀着墨形成纪念印刷品的真实温度。

### 适合这些情况

- **想把照片变成私人纪念图册：** 用少量彩色单线保留人物、地点、物件和关系中真正值得记住的线索。
- **不喜欢黑描边卡通感：** 让 2–4 种来源于原图的专色色线直接承担轮廓、结构和信息层级。
...
> 用 XXD Panel 081 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 081 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅像旅行图册、地图与建筑图解交汇的限色私人纪念海报 |
| **一眼可辨的特点** | 近等粗彩色线、开放轮廓、密度分层、套印偏差与大面积暖白纸面 |
| **它如何尊重你的输入** | 只留下决定身份与情绪的线索；专色从原图最鲜活、最有记忆度的颜色中提取 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-081`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-081-prompt.en.md) · [中文运行适配器](references/xxd-panel-081-prompt.zh-CN.md)
...
**关键词：** 等粗手绘 monoline · 彩色线条编码 · 线密度层级 · 2–4 色专印 · Risograph 颗粒 · 私人纪念叙事
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 081 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-081 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-081 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-081 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 081；公共设置只解析一次
/xxd-panel-081 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–081；当前项目以粗体标出。
