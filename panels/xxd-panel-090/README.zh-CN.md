...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 090 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 090 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 090 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 090 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 090 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 090 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 090 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 090 新增上下样张 4"></td>
  </tr>
</table>
...
普通物体说明图只解释“它是什么”，传统流程图和科技 UI 又把意义压扁成固定模板；090 关注的是照片背后的关系、过程、状态与隐喻。

**Panel 090** 把照片视为一个概念中心，将动作、方向、空间、情绪、关系与隐喻拆成文字、节点、轨迹、区域、箭头和连接关系。文字语义、几何关系与手工 mark-making 共同构成介于绘画、图表和思维笔记之间的艺术地图。

### 适合这些情况

- **想表达照片背后的思维过程：** 把动作、方向、关系和情绪组织成概念中心与视觉路径。
- **需要艺术化图式而非科学图表：** 使用三角结构、折线、交叉轴线、放射线、等高线和场域轨迹。
- **追求理性与身体感的张力：** 用清晰几何骨架叠加重复短线、刻度、符号、点和轻微手写偏移。
...
> 用 XXD Panel 090 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 090 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一张把照片关系、过程、状态与隐喻组织成图式的视觉思维地图 |
| **一眼可辨的特点** | 文字节点、几何骨架、轨迹箭头、方向性短线、点与手工偏移、大面积留白 |
| **它如何尊重你的输入** | 以主体、动作、空间和叙事关系为概念中心，不把原图做成物体说明图 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-090`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-090-prompt.en.md) · [中文运行适配器](references/xxd-panel-090-prompt.zh-CN.md)
...
**关键词：** 视觉思维 · 概念映射 · 图式绘画 · 实验字体 · 视觉记谱 · 手工痕迹
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 090 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-090 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-090 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-090 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 090；公共设置只解析一次
/xxd-panel-090 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–090；当前项目以粗体标出。
