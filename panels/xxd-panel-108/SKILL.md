---
name: xxd-panel-108
description: "Turn each source photograph into a strict 50:50 top-bottom or left-right poster whose transformed half distils the subject into contemporary folk-art paper-cut collage with print texture and generous whitespace. Use when the user invokes xxd-panel-108 or requests this exact independent Panel style."
---
...
Write every selected final PNG directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-108/` (or the explicit `--out` root). Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-108-prompt.zh-CN.md` and `.en.md` — delivery adapter notes matching the family runtime blocks
