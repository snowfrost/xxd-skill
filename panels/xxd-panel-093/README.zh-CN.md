...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 093 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 093 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 093 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 093 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 093 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 093 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 093 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 093 新增上下样张 4"></td>
  </tr>
</table>
...
**Panel 093** 从照片中保留主体、轮廓、姿态与叙事关系，再把它们概括成矩形、梯形、三角形、圆形和少量自由块面。空间不靠真实透视与复杂阴影，而靠前后遮挡、尺度变化、色块叠压与高低关系形成；明亮取色、轻微手绘边缘和哑光水粉质感，让结果兼具剪纸般的平面感与独立出版物的完成度。

### 适合这些情况

- **想把复杂照片变得一眼可读：** 合并重复元素，以最少形状保存最关键的身份特征。
- **需要鲜活而不廉价的色彩：** 从原图最明亮、最有记忆点的颜色出发，重新组织有限而清楚的综合色块。
- **追求现代主义的质朴完成度：** 以扁平空间、轻微不规则边缘和哑光水粉／丙烯覆盖感，避开光滑矢量与模板感。
...
> 用 XXD Panel 093 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 093 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅明快、克制、质朴而高级的现代主义几何平涂插画 |
| **一眼可辨的特点** | 少量大块几何形、扁平空间、鲜明综合色、轻微手绘边缘与哑光颜料肌理 |
| **它如何尊重你的输入** | 保留主体身份、姿态和叙事关系，并从照片的色彩记忆出发重组，而不是强行套入城市题材 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-093`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-093-prompt.en.md) · [中文运行适配器](references/xxd-panel-093-prompt.zh-CN.md)
...
**关键词：** 几何平涂 · 扁平空间 · 明快取色 · 手绘边缘 · 哑光水粉 · 中世纪现代主义编辑插画
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 093 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-093 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-093 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-093 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 093；公共设置只解析一次
/xxd-panel-093 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–093；当前项目以粗体标出。
