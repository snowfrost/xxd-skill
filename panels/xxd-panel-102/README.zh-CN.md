...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 102 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 102 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 102 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 102 样张 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 102 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 102 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 102 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 102 新增上下样张 4"></td>
  </tr>
</table>
...
**Panel 102** 适合想保留照片主体辨识度，又希望摆脱杂乱背景、写实复刻和廉价滤镜的人。它把主体最重要的轮廓、结构、姿态与关系提炼成少量几何色块，在清透留白中形成安静、现代而有生命力的治愈系编辑海报。

### 适合这些情况

- **照片信息太多，主体不够突出：** 只保留最有辨识度的轮廓、结构、姿态与叙事关系，压缩成清楚的几何形。
- **想要现代高级感，又不想冷硬：** 用正负形、层级和大面积留白建立专业秩序，同时保留柔和、温暖的治愈气质。
- **常见滤镜让颜色脏、暗或廉价：** 从原图提炼明亮、有生命力的颜色，提高明度与纯净度，避免荧光和塑料感。
...
> 用 XXD Panel 102 帮我处理这张图，先推荐最合适的构图和尺寸。

<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 102 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一张以少量几何形、清透色块和理性留白承载照片记忆的现代编辑海报 |
| **一眼可辨的特点** | 几何化主体、平面色块、清晰轮廓、细线微注释、浅色背景与大面积留白 |
| **它如何尊重你的输入** | 保留原物最核心的识别特征、姿态和关系，使设计部分仍能一眼对应当前照片 |
| **可以用在哪里** | 品牌内容、展览海报、建筑与空间主题、人物／宠物纪念、封面、社交内容和多端壁纸 |
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-102`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-102-prompt.en.md) · [中文运行适配器](references/xxd-panel-102-prompt.zh-CN.md)
...
**关键词：** geometric abstraction · flat colour · clear contours · positive–negative shape · generous whitespace · healing editorial system
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 102 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-102 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-102 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-102 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 102；公共设置只解析一次
/xxd-panel-102 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
XXD Panel 当前系列已更新至 001–112；每个 Panel 仍保留独立的原始提示词与审美逻辑。下表是本项目发布时的历史目录，连续列出 001–102；当前项目以粗体标出。
